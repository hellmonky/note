...
[OSI的七层参考模型](https://zh.wikipedia.org/wiki/OSI%E6%A8%A1%E5%9E%8B)。
传统的开放式系统互连参考模型（OSI模型），是一种通信协议的7层抽象的参考模型,其中每一层执行某一特定任务。该模型的目的是使各种硬件在相同的层次上相互通信。这7层是:
> - 1. 物理层
> - 2. 数据链路层
> - 3. 网路层
> - 4. 传输层
...
和传统的OSI模型不同，TCP/IP通讯协议采用了4层的层级结构，每一层都呼叫它的下一层所提供的网络来完成自己的需求。这四个层次为：
...
从上述例子可以看出，通过了解一个网络通信的实际层次有助于帮助我们分析整个网络通信的流程。
