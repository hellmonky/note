...
Rust中包含很多原生整数类型，i开头的表示有符号整数，u开头的代表无符号整数。并且整数的大小可以为8，16，32和64位。
Rust会自动推断数据的类型，所以可以并不需要上述代码一样指定类型，当然指定类型也是非常严格的方式，对于底层编程非常有用。

## 可变性（Mutability）
变量绑定默认是不可变的（immutable），例如：
```Rust
let x = 5;
x = 10;
```
...
error: re-assignment of immutable variable `x`
     ...
     ^~~~~~~
```
如果想要一个变量是可绑定的，可以使用mut来对变量进行描述，例如：
```Rust
