...
##### 类型：
关于类型声明，Haskell会自动进行推断，不必声明之。如果非要声明类型，可用 :: 符号明确指出。符号 :: 读作 ”具有类型”：
```haskell
5 :: Int
5 :: Double
```
...
```
这个返回结果表示ghci对当前输入内容的类型推导，说明：[(1,'a'),(2,'b'),(3,'c'),(4,'d'),(5,'e')]具有类型Num t => [(t, Char)]。
...
##### 函数定义：
函数式程序设计语言的核心就是函数，这一点在lisp和scheme的学习中已经非常清晰明确的认识到了（可以和C中的函数理解为处于同一阶段，因为还没有设计面向对象等）。
在定义函数之前，先看一下函数的调用方法：
要调用一个函数，先写出它的名字，后接函数的参数。函数的参数不需要用括号来包围，参数和参数之间也不需要用逗号来隔开，只需要用空格进行分割就可以了。
在上述章节中都一直在使用函数，例如：
```haskell
succ 5
round 6.59
...
doubleMe x = x + x
```
在这个定义中，没有明确的说明函数的类型，可以被haskell自动推导。
函数的定义语法为：
```shell
name x1 x2 ... xk = e
